...
**Решение:** Встроенная классификация в runner через флаг `--classify`. Читает SpendLogs для персоны, группирует по session_id, запускает `daily_classification()` на каждую сессию, записывает в PredictTable. Также сохранён `--option-b` для детерминированных значений.
...
Runner переписан для безопасного итеративного запуска экспериментов.

**Проблема:** данные предыдущих итераций могут смешиваться с текущими в БД. SpendLogs удваиваются, temporal.py видит двойные сообщения, baselines загрязняются.

**Решение:** auto-archive + mandatory clean. Каждый non-dry-run автоматически:
1. Экспортирует текущие данные персоны в CSV (`experiments/results/{persona}/iteration_{N}/`)
2. Удаляет все данные персоны из БД
3. Генерирует/классифицирует/агрегирует с чистого листа
...
experiments/results/
  viktor/
    iteration_1/
      spendlogs.csv, predict.csv, behavioral.csv,
      daily_summaries.csv, events.csv, aggregation.csv, summary.txt
    iteration_2/
      ...
```

**Новые флаги:**
- `--classify` — запускает `daily_classification()` на SpendLogs -> PredictTable (без Langfuse)
- `--reaggregate` — сохраняет SpendLogs + PredictTable, чистит только behavioral, перезапускает Stages 1-4

**Типичный workflow:**
```bash
# Итерация 1
PYTHONPATH=experiments py -m synthetic.runner --persona sara --classify \
  --plm-model <model> --clm-model <model>
# -> Score: 10/14 (71%)

# Правим risk_engine.py...
# Итерация 2 (полная перегенерация)
PYTHONPATH=experiments py -m synthetic.runner --persona sara --classify \
  --plm-model <model> --clm-model <model>
# -> auto-archives iteration 1 -> cleans DB -> Score: 14/14 (100%)

# Правим behavioral_llm.py...
# Итерация 3 (только ре-агрегация, диалоги те же)
PYTHONPATH=experiments py -m synthetic.runner --persona sara --reaggregate
# -> archives iteration 2 -> re-runs pipeline -> Score: 14/14 (100%)
```
...
2. Запустить 1-2 сложных персоны (Viktor, Amanda)
3. Итеративно фиксить проблемы до Score ~85%+
4. Запустить Experiment 2 (boundary tests) и Experiment 3 (middleware)
5. Полная валидация по всем 11 персонам
6. eRisk корреляционный анализ
...
4a27c35 feat: iterative experiment runner with auto-archive
```
